...
def cleanup_logs_test_mode(folder_retention=2, file_retention=7, dry_run=True):
    ...
    try:
        ...
        if not dry_run:
            ...
            # Rescan to verify deletion
            remaining_folders = [f for f in os.listdir(TEST_PATH)
                               if os.path.isdir(os.path.join(TEST_PATH, f)) and len(f) == 6 and f.isdigit()]
            remaining_broken = [f for f in os.listdir(TEST_PATH)
                              if os.path.isfile(os.path.join(TEST_PATH, f)) and os.path.getsize(os.path.join(TEST_PATH, f)) == 0]
            remaining_loose = [f for f in os.listdir(TEST_PATH)
                             if os.path.isfile(os.path.join(TEST_PATH, f)) and os.path.getsize(os.path.join(TEST_PATH, f)) > 0]

            log(f"Remaining folders: {len(remaining_folders)} ({', '.join(remaining_folders) if remaining_folders else 'none'})")
            log(f"Remaining broken files: {len(remaining_broken)}")
            log(f"Remaining loose files: {len(remaining_loose)}")
            log(f"\n[NOTE] Click 'Clean Logs' button again to generate fresh test data")
